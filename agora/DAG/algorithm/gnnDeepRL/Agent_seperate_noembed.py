...
class Agent(object):
    ...
    def update(self,values,rewards,Qvalue,logprobs,entropy_term,values_policy,actions):

        Qvals = np.zeros_like(values_policy)
        mask = torch.ones(len(rewards), dtype=torch.bool, device=device)  # Initialize mask as a tensor
        ...
        print(f'values_policy shape is {values_policy.shape}')
        print(f'values shape is {values.shape}')
        print(f'rewards shape is {rewards.shape}')
        print(f'Qvals shape is {Qvals.shape}')

        for t in reversed(range(len(rewards))):
            # print(f'reward is {rewards[t]}')
            if logprobs[t][0]==float(-1e-20):
                mask[t] = False
                # Qvals[t]=torch.tensor(0).unsqueeze(0)
                # values[t]=torch.tensor(0).unsqueeze(0)
                # values_policy[t]=torch.tensor(0).unsqueeze(0)

            else:
                Qval = rewards[t] + self.gamma * Qvalue
                Qvals[t] = Qval

        filtered_qvalues=Qvals[mask]
        print(f'values shape {values.shape}')
        filtered_values=values[mask]
        filtered_values_policy=values_policy[mask]

        filtered_logprobs = [logprobs[i] for i in range(len(logprobs)) if mask[i]]
        if not filtered_logprobs:
            return None,None
        log_probs = torch.stack(filtered_logprobs)
        entropy_term = torch.FloatTensor(entropy_term).to(device).requires_grad_(True)

        # update actor critic
        values = torch.FloatTensor(filtered_values)
        Qvals = torch.FloatTensor(filtered_qvalues).to(device).requires_grad_(True)
        print("Grad enabled for Qvals:", Qvals.requires_grad)
        print(f'Qvals shape {Qvals.shape}')

        values_no_grad = torch.FloatTensor(filtered_values_policy)
        actions = torch.tensor(actions, dtype=torch.int64).to(device)

        #
        # print(f'Qvals  is {Qvals}')
        ...
        # print(f'Actor loss is {actor_loss}')
        #
        # critic_loss = 0.5 * advantage.pow(2).mean()
        # print(f'critic_loss  is {critic_loss}')
        #
        # ac_loss = -(actor_loss + critic_loss + 0.15 * entropy_term)
        # print(f'final loss is {ac_loss}')
        ...
        advantages = Qvals - values_no_grad
        #
        # with torch.no_grad():
        #     values = value_func(states)
        # actions = torch.tensor(actions, dtype=torch.int64).to(device)
        ...
        entropy_coef = 0.15  # Small positive value to control the impact of entropy
        # print(f'entropy_term is {entropy_term}')
        # print(f'entropy_term is {entropy_term}')

        # entropy_term = (entropy_term - entropy_term.mean()) / (entropy_term.std() + 1e-8)
        # print(f'entropy_term is {entropy_term}')

        ent=entropy_coef * entropy_term.mean()
        log_probs = -F.cross_entropy(log_probs, actions, reduction="none")
        # print(f'pi_loss is {log_probs* advantages}')
        # print(f'actions shape is {actions.shape}')
        # print(f'ent is {ent}')

        pi_loss = -log_probs * advantages -ent
        # print(f'ent is {ent}')
        # print(f'advantages shape is {advantages.shape}')
        #
        # print(f'pi_loss shape is {pi_loss.shape}')

        # print(f'pi_loss is {pi_loss}')
        pi_loss.mean().backward()
        self.optimizer.step()

        return pi_loss.mean(),vf_loss.mean()

    # def update_batches(self, values, rewards, Qvalue, logprobs, entropy_term, values_policy, actions, batch_size=64):
    #     # Convert inputs to PyTorch tensors
    #     rewards = torch.tensor(rewards, device=device)
    #     values = torch.FloatTensor(values).to(device)
    #     values_policy = torch.FloatTensor(values_policy).to(device)
    #     logprobs = torch.stack(logprobs).to(device)
    #     actions = torch.tensor(actions, dtype=torch.int64).to(device)
    #     entropy_term = torch.FloatTensor(entropy_term).to(device)
    #
    ...
    #
    #     # Compute Q-values
    #     Qvals = torch.zeros_like(values_policy, device=device)
    #     for t in reversed(range(len(rewards))):
    #         if logprobs[t][0] != float(-1e-20):
    #             Qval = rewards[t] + self.gamma * Qvalue
    #             Qvals[t] = Qval
    #
    #     # Compute advantages
    #     advantages = Qvals - values_policy
    #     advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)
    #
    #     # Batch processing
    #     dataset = torch.utils.data.TensorDataset(logprobs, values, Qvals, advantages, actions)
    #     dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)
    #
    #     # Update actor and critic
    #     for batch_logprobs, batch_values, batch_Qvals, batch_advantages, batch_actions in dataloader:
    #         # Critic loss
    #     vf_loss = F.mse_loss(values, batch_Qvals)
    #     self.optimizer_critic.zero_grad()
    #     vf_loss.backward()
    #     torch.nn.utils.clip_grad_norm_(self.optimizer_critic.param_groups[0]['params'], max_norm=0.5)
    #     self.optimizer_critic.step()
    #
    #     # Actor loss
    #     log_probs = -F.cross_entropy(batch_logprobs, batch_actions, reduction="none")
    #     pi_loss = -(log_probs * batch_advantages).mean() - 0.09 * entropy_term.mean()
    #     self.optimizer.zero_grad()
    #     pi_loss.backward()
    #     torch.nn.utils.clip_grad_norm_(self.optimizer.param_groups[0]['params'], max_norm=0.5)
    #     self.optimizer.step()
    #
    #     return pi_loss.mean().item(), vf_loss.mean().item()
