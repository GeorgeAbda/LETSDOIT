...
sys.path.append('..')
from agora.DAG.algorithm.gnnDeepRL.Agent_seperate_noembed  import Agent
...
for job_chunk in range(2,15):
    name = "Without entropy A2C on policy Montecarlo " + str(job_chunk) + " /20"
    ...
    #     episode = Episode(machine_configs, jobs_configs, algorithm, None,0)
    ...
    for itr in range(n_iter):
        ...
        for e in range(20):
            ...
            average_energies.append(total_reward * episode.simulation.env.now/3600000)


            wandb.log({
                "Total Energy ": total_energy * episode.simulation.env.now/3600000
            })
            ...
            for node in trajectory:
                ...
                probs_ = logits.tolist()
                # print(type(node.prob.detach()))
                if node.prob.detach()[0] == float(-1e-20):
                    # print('netaaaadewww')
                    continue
                # print(f'logits are {logits} ')
                action_probs_over_time.append(probs_)

                policy_entropy = -np.sum(logits * np.log(logits + 1e-10))
                # print(f'policy entropy is {policy_entropy} ')
