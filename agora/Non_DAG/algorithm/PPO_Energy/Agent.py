...
class Agent(object):
    def __init__(self, gamma, epsilon, reward_to_go, nn_baseline, normalize_advantages, entropy_coefficient=0.01,lr=0.001,batch_size=1000,model_save_path=None
                 ):
        super().__init__()
        ...
        self.optimizer = Adam(self.brain.parameters(), lr=self.lr)
    ...
    def _loss(self, X, y, advantage,mask):
        ...
        logits = self.brain(X).to(dtype=torch.float32)
        logits_ = logits * mask + (1 - mask) * 1e-8

        logits2 = self.neighbor(X).to(dtype=torch.float32)
        logits2_ = logits2 * mask + (1 - mask) * 1e-8
        ...
        logprob1 = logprobs[0][y[0]]
        logprob2 = logprobs2[0][y[0]]
        ...
        for observations, actions, advantages,masks in zip(all_observations, all_actions, all_advantages,all_masks):
            ...
            for observation, action, advantage,mask in zip(observations, actions, advantages,masks):
                ...
                if cnt % self.batch_size == 0:
                    ...
                    # for param in self.neighbor.parameters():
                    #     param.data = param.data.to(dtype=torch.float32)
                    #     print('done with parameter neighbors')
                    #     param.requires_grad = False
                    if self.count % 5 == 0:
